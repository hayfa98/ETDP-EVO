...
class Route:
    def __init__(self, route, demand, distance, RouteDuration, time_window_feasible, routeEarlyViolation, routeLateViolation, routeViolation):
        ...
        self.num_satisfied_customers = 0
    ...
    def getNumSatisfiedCustomers(self):
        return self.num_satisfied_customers
    ...
    def setNumSatisfiedCustomers(self, num_satisfied_customers):
        self.num_satisfied_customers = num_satisfied_customers   
...
class Individual:
    ...
    def __lt__(self, other):
        ...
        if not self.isFeasible and other.isFeasible:
            return False
        return self.economic_cost < other.economic_cost      
...
class RouteScheduler:
    ...
    def heuristic_amelioration(tab):
        ...
        # make a deep copy so original tab is unchanged
        new_tab = copy.deepcopy(tab)
        
        if random.random() > 0.5:
            # 2-opt style: swap second-last customers of two routes
            x, y = random.sample(range(len(tab)), 2)
         
            # access the route list inside the Route object
            route_x = new_tab[x].getRoute()
   
            route_y = new_tab[y].getRoute()
            route_x[-2], route_y[-2] = route_y[-2], route_x[-2]
            
             
        else:
            # OR-opt style: swap random customers across routes
            ...
            if len(route_x) > 2 and len(route_y) > 2:
                a = random.randint(1, len(route_x) - 2)
                c = random.randint(1, len(route_y) - 2)
                # swap customers
...
class Insertion:
    ...
    def insertionCost(self, routes, customerID, routeLoc, index):
        ...
        if len(augmentedRoute)==1:
            ...
            return self.metrics.evaluateSocialCostRoute(augmentedRouteObject, self.depot)
        ...
        return (self.metrics.evaluateSocialCostRoute(augmentedRouteObject, self.depot) 
            - self.metrics.evaluateSocialCostRoute(oldRouteObject, self.depot))
...
class Metrics:
    ...
    def evaluateSocialCostRoute(self, route, depotID):
        """
        Returns the Gini coefficient (0..1) of workloads across routes.
        0 => perfectly equal workloads, 1 => totally unequal.
        """
        ...
        num_satisfied_customers = self.getNumSatisfiedCustomersRoute(route, depotID)
    
        customer_satisfaction = num_satisfied_customers / len(route.getRoute())
    ...
    def getNumSatisfiedCustomersRoute(self, route, depot):
    ...
    """ def EconomicCostBetween(self, fromCustomer, toCustomer, currentTime):
        tech_hourly_wage = 20
        # Travel
...
class Selection:
    ...
    def runTournamentSelection(player1, player2, fitness_bias):
        ...
        if random.random() < fitness_bias:
            if player2.getSocialCost() > player1.getSocialCost():
...
class Algorithm:
    ...
    def run(self, historyShortestDistance):
        ...
        print(f"Generation: 0  |  Population size: {len(population)}  | Average total distance: {average_distance} | Best individual: {population[0].getSocialCost()}")

        # Sort population based on fitness (ascending order)
        population.sort(key=lambda individual: individual.getSocialCost()) 
    
     

        # Add the fitness of the best individual to the history
        historyShortestDistance.append(population[0].getSocialCost())
        ...
        for generation in range(1, self.numberOfGenerations + 1):
            ...
            if generation % 100 == 0:
              ...
              f"  | Average total distance: {averageDistance}"
              f"  | Best individual distance {population[0].getSocialCost()}"
              f"  | Best individual is feasible: {population[0].getFeasibility()}"
              f"  | Worst individual distance {population[-1].getSocialCost()}")
    ...
    def evaluatePopulation(self, population):
        ...
        for individual in population:
                ...
                fitness = self.metrics.getTotalDistance(individual)
                minmax_workload = self.metrics.getWorkload(individual)
                num_tech = self.metrics.getNumTech(individual)
                ...
                individual.setMinMaxWorkload(minmax_workload)
                individual.setNumTech(num_tech)
...
manager = Manager(r"C:\Users\Haifa Zaidi\OneDrive\Pictures\Research\data\test_data1.txt", 0.5)
...
def print_individual(individual):
    ...
    result += (
        ...
        f"Feasibility: {individual.getFeasibility()} | \n"
