...
class Customer:
    def __init__(self, id, x, y, service_time, ready_time, due_time, demand):
        """
    ...
    # Setters
    def setX(self, x):
        self.x = x  # Used to shift coordinates before plotting
...
class Manager:
    ...
    def readData(self, problemFilepath):
        try:
            with open(problemFilepath, 'r') as fileReader:
                ...
                nVehicles, nCustomers, nDepots = map(int, firstLine.split()[1:4])
                ...
                for _ in range(nCustomers):
                    ...
                    customer = Customer(customerId, x, y, service_time, ready_time, due_time, demand)
                    self.customers.append(customer)
    ...
    def assignCustomersToDepotsRandomly(self):
        depots = [CrowdedDepot(depot) for depot in self.copyDepots()]
        depotCapacities = {depot.getId(): math.ceil(sum(customer.getDemand() for customer in self.customers) / len(depots)) for depot in depots}

        customers = self.customers.copy()
        random.shuffle(customers)  # Shuffle customers to ensure randomness

        for customer in customers:
            possibleDepots = [depot for depot in depots if depotCapacities[depot.getId()] > 0]  # Only consider depots with space

            if not possibleDepots:
                raise ValueError("All depots are full before assigning all customers. Adjust capacity limits.")

            assignedDepot = random.choice(possibleDepots)  # Assign randomly from available depots
            assignedDepot.addCustomer(customer)
            customer.addPossibleDepot(assignedDepot.getId())
            depotCapacities[assignedDepot.getId()] -= customer.getDemand()  # Decrease capacity

            # Borderline logic: Keep track of other close depots
            depotDistances = {depot.getId(): Euclidian.distance([customer.getX(), customer.getY()], [depot.getX(), depot.getY()])
                          for depot in depots}
            shortestDistance = depotDistances[assignedDepot.getId()]

            isBorderlineCustomer = False
            for depotId, distance in depotDistances.items():
                if depotId != assignedDepot.getId() and (distance - shortestDistance) / shortestDistance < self.borderlineThreshold:
...
class Individual:
    def __init__(self, initial_chromosome: Dict[int, List['Route']]):
        ...
        """
        self.fitness = 0  # Fitness is initially set to infinity
        self.minmax_workload = 0
        self.num_tech = 0
        self.DepotServiceDuration = 0
    ...
    def getMinMaxWorkload(self):
        return self.minmax_workload
    
    def getNumTech(self):
        return self.num_tech
    
    def getDepotServiceDuration(self):
        return self.DepotServiceDuration
    ...
    def setNumTech(self, num_tech: int):  
        self.num_tech = num_tech
    
    def setMinMaxWorkload(self, minmax_workload: float):  
        self.minmax_workload = minmax_workload
        
    def setTimeWindowViolation(self, TimeWindowViolation):
        self.TimeWindowViolation = TimeWindowViolation
    ...
    def __str__(self):
        ...
        for key, chromosome_depot in self.chromosome.items():
            ...
            for route in chromosome_depot:
                result += str(route.getRoute()) + " " + str(route.getDemand()) + " - "
             # Remove the last ' - '
            result += "| "
    ...
    def getClone(self):
        ...
        cloned_individual.fitness = self.fitness  # Copy the fitness value
        cloned_individual.minmax_workload = self.minmax_workload
        cloned_individual.num_tech = self.num_tech
...
class Initializer:
    @staticmethod
    def init(populationSize, depots, metrics):
        """
    ...
    def getInitialRoutes(depot, metrics, method, manager):
        ...
        if method == "savings":
           ...
           # Step 1: compute savings
            ...
            for route in routes.values():
                ...
                time_early_violation, time_late_violation, time_violation = metrics.calculateTimeWindowViolation(depot.getId(), route)
        ...
        # ------------------------------------------------------------------
            ...
            for c in customers:
                ...
                if current_load + demand <= depot.getMaxVehicleLoad():
                   ...
                   time_early_violation, time_late_violation, time_violation = metrics.calculateTimeWindowViolation(depot.getId(), current_route) 
                   ...
                   final_routes.append(Route(current_route, current_load, dist, route_duration, time_feasible, time_early_violation, time_late_violation, time_violation))
        ...
        # ------------------------------------------------------------------
            ...
            for route in routes.values():
                ...
                time_feasible = metrics.checkTimeFeasibility(depot.getId(), route)
                time_early_violation, time_late_violation, time_violation = metrics.calculateTimeWindowViolation(depot.getId(), route) 
                final_routes.append(Route(route, new_load, route_distance, route_duration, time_feasible, time_early_violation, time_late_violation, time_violation))
...
class Insertion:
    ...
    def insertionCost(self, routes, customerID, routeLoc, index):
        ...
        if len(augmentedRoute) == 1:
            return 2 * Euclidian.distance([self.depot.getX(), self.depot.getY()], [customer.getX(), customer.getY()])
        ...
        if index == 0 or index == len(augmentedRoute) - 1:
            ...
            distanceToDepot = Euclidian.distance([self.depot.getX(), self.depot.getY()], [customer.getX(), customer.getY()])
            distanceToOther = Euclidian.distance([customer.getX(), customer.getY()], [otherCustomer.getX(), otherCustomer.getY()])
            distanceDepotToOther = Euclidian.distance([self.depot.getX(), self.depot.getY()], [otherCustomer.getX(), otherCustomer.getY()])
        ...
        distanceToCustomerBefore = Euclidian.distance([customer.getX(), customer.getY()], [customerBefore.getX(), customerBefore.getY()])
        distanceToCustomerAfter = Euclidian.distance([customer.getX(), customer.getY()], [customerAfter.getX(), customerAfter.getY()])
        distanceBetweenBeforeAfter = Euclidian.distance([customerBefore.getX(), customerBefore.getY()], [customerAfter.getX(), customerAfter.getY()])
...
class Metrics:
    ...
    def areRoutesFeasible(self, depot, routes):
        ...
        for route in routes:
            ...
            if duration > maxDuration or demand > depot.getMaxVehicleLoad():
                return False
    ...
    def CalculateRouteDuration(self, depotID, route):    
        ...
        for i in range(1, len(route)):
            ...
            travelTime = Euclidian.distance([prevCustomer.getX(), prevCustomer.getY()],[customer.getX(), customer.getY()]) 
...
class Selection:
    ...
    @staticmethod
    def runTournamentSelection(player1, player2, fitness_bias, population, s1):
        """
        ...
        """
        fit1 = s1.evaluateIndividual(population, player1)
        fit2 = s1.evaluateIndividual(population, player2)
...
class Algorithm:
    ...
    def run(self, historyShortestDistance):
        ...
        population = Initializer.init(self.populationSize, crowdedDepots, self.metrics)
        print(f"\nInitial population size: {len(population)}")
        
        # Evaluate fitness
        average_fitness = self.evaluatePopulation(population)
        #print(average_fitness)
        
        print_individual(population[0])
        self.evaluateFeasibility(population)
        print(f"Generation: 0  |  Population size: {len(population)}  | Average combined fitness: {average_fitness} | Best individual: {self.evaluateIndividual(population,population[0])}")

        # Sort population based on fitness (ascending order)
        #population.sort(key=lambda individual: individual.getFitness()) 
        population.sort(key=lambda individual:self.evaluateIndividual(population,individual))

        # Add the fitness of the best individual to the history (combined fitness)
        historyShortestDistance.append(self.evaluateIndividual(population,population[0]))
        ...
        for generation in range(1, self.numberOfGenerations + 1):
            ...
            for individual in population:
                fitness.append(self.fitness_tuple(individual))
            ...
            for i in range(len(fitness)):
                for j in range(len(fitness)):
                    if(j not in domInd and i!=j):
                        if(self.checkDominance(fitness[i], fitness[j])):
                            #print(fitness[i][0], fitness[j][0], "True")
                            #print("True", fitness[i], fitness[j])
                            dominated.append(population[j])
                            domInd.append(j)
            ...
            #print('pop',len(set1))
            #print('dominated',len(set2))

            # Perform set difference to get elements in set1 but not in set2
            result_set = set1 - set2

            #print(result_set)

            # Convert the result set back to a list
            population = list(result_set)

            #print("Pareto dominate", len(population))
            ...
            # Select 2 competitor pairs
                ...
                parent1 = Selection.runTournamentSelection(competitorPair1[0], competitorPair1[1], self.fitnessBias, population, self)
                parent2 = Selection.runTournamentSelection(competitorPair2[0], competitorPair2[1], self.fitnessBias, population, self)
            ...
            averageCombFitness = self.evaluatePopulation(offspring)
            population = offspring

             #Maintaining pop size constant
            if(len(population)>=self.populationSize):
                population = population[0:self.populationSize]
            elif len(population)<self.populationSize:
                #Generate extra population
                #newPop += population(n-len(newPop), cp_net.domain, cp_net, constTable)
                x = Initializer.init(self.populationSize - len(population), crowdedDepots, self.metrics)
                population += x 
            ...
            if generation % 100 == 0:
              ...
              f"  | Average combined fitness: {averageCombFitness}"
              f"  | Best individual: {self.evaluateIndividual(population,population[0])}"
              f"  | Best individual is feasible: {population[0].isFeasible}"
              f"  | Worst individual: {self.evaluateIndividual(population,population[-1])}")
        ...
        fCounter = 0
        for individual in population:
            if individual.isFeasible:  # Assuming isFeasible() is a method that returns a boolean
                fCounter += 1
        ...
        print(f"Result is feasible {bestIndividual.isFeasible}")
    ...
    def evaluatePopulation(self, population):
        totalFitness = 0
        maxEco, maxEnv, maxSoc  =[float('-inf'), float('-inf'),float('-inf')]
        minEco, minEnv, minSoc = [float('inf'), float('inf'),float('inf')]

        #Considering specified weights:
        w1 = 0.6 #distance
        w2 = 0.3 #workload
        w3 = 0.1 #vehicle
        ...
        for individual in population:
                ...
                if (maxEco < economic_cost):
                    maxEco = economic_cost
                if (minEco > economic_cost):
                    minEco = economic_cost

                if (maxEnv < environmental_cost):
                    maxEnv = environmental_cost
                if (minEnv > environmental_cost):
                    minEnv = environmental_cost
                
                if (maxSoc < social_cost):
                    maxSoc = social_cost
                if (minSoc > social_cost):
                    minSoc = social_cost
                
                #print(fitness, minmax_workload, num_tech, "originals...")

        #print(maxFit, minFit, maxWork, minWork, maxVeh, minVeh, ",,,,valkues") 
        ...
        for individual in population:
                ...
                if (maxEco == minEco):
                    normalizedEconomic = 0
                else:
                    normalizedEconomic = (economic_cost - minEco) / (maxEco - minEco)
                
                if (maxEnv == minEnv):
                    normalizedEnvironmental = 0
                else:
                    normalizedEnvironmental = (environmental_cost - minEnv) / (maxEnv - minEnv)
                
                if (maxSoc == minSoc):
                    normalizedSocial = 0
                else:
                    normalizedSocial = (social_cost - minSoc) / (maxSoc - minSoc)

                totalFitness += (w1*normalizedEconomic)+ (w2*normalizedEnvironmental) + (w3*normalizedSocial)
            #else:
                #totalDistanceForPop += individual.getFitness()
        return totalFitness/len(population)
    

    #Getting the fitness of one individual
    def evaluateIndividual(self, population, ind):
        fitness = 0
        maxEco, maxEnv, maxSoc  =[float('-inf'), float('-inf'),float('-inf')]
        minEco, minEnv, minSoc = [float('inf'), float('inf'),float('inf')]

        #Considering specified weights:
        w1 = 0.6 # economic cost
        w2 = 0.3 # environmental cost
        w3 = 0.1 # social cost
        ...
        economic_cost = self.metrics.evaluateEconomicCost(ind)
        environmental_cost = self.metrics.evaluateEnvironmentalCost(ind)
        social_cost = self.metrics.evaluateSocialCost(ind)
        ...
        fitness = (w1*normalizedEconomic)+ (w2*normalizedEnvironmental) + (w3*normalizedSocial)             

        return fitness
    
    # Function to get a individual ftiness as tuple       
    def fitness_tuple(self, ind):
        economic_cost = self.metrics.evaluateEconomicCost(ind)
        environmental_cost = self.metrics.evaluateEnvironmentalCost(ind)
        social_cost = self.metrics.evaluateSocialCost(ind)
        
        ind.setEconomicCost(economic_cost)
        ind.setEnvironmentalCost(environmental_cost)
        ind.setSocialCost(social_cost)    
    ...
    #return True if seq2 is dominated by seq1
    def checkDominance (self, seq1, seq2):
        p1 = seq1[0]
        q1 = seq1[1]
        r1 = seq1[2]
        ...
        # At least one of the values must be strictly less
...
manager = Manager(r"C:\Users\Haifa Zaidi\OneDrive\Pictures\Research\data\test_data1.txt", 0)
...
def print_individual(individual):
    ...
    result += (
        ...
        f"Environmental Cost: {individual.getEnvironmentalCost()} | "
